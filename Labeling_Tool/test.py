...
class Application(Global):
    ...
    def __init__(self, parent, image_path, xml_path, frames):
        # initialize frame
    ...
    def initUI(self):
        ...
        # image
        loaded_img = Image.open(self.image_path)

        # Resize Image
        resized_image = loaded_img.resize((1200, 1000))
        render = ImageTk.PhotoImage(resized_image)
        self.canv.xview_moveto(0)
        self.canv.yview_moveto(0)
        self.canv.create_image(0, 0, image=render, tags='img')
    ...
    def on_mouse_up(self, event):
        if self.item:
            ...
            roi = self.frames[Global.frameNumber].crop(box) # region of interest
