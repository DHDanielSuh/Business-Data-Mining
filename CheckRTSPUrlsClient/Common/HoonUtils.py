...
def crop_roi(img, roi, imshow_sec=-1):
    ...
    if imshow_sec >= 0:
        ...
        imshow(hstack_img, desc="crop_img: {} -> {}".format(img.shape[1::-1], crop_img.shape[1::-1]), pause_sec=imshow_sec)
...
def transform_quadrilateral_to_rectangle(quad, algo='max', margin=16):
    ...
    if algo == 'max':
        ...
        return [[t[0][0]-margin, t[0][1]-margin], [t[1][0]+margin, t[1][1]+margin]]
    else:
        return None
...
def get_corners2_from_corners4(corner, corner_type='zigzag', method='maximum', margin=0, flatten_=False):
    """
    :param corner:
    :param corner_type:
